...
ExprRef ConstraintBase::VectorsParallel(int eq, ExprVector a, ExprVector b) {
...
ExprRef ConstraintBase::PointLineDistance(hEntity wrkpl, hEntity hpt, hEntity hln)
{
    ...
    if(wrkpl.v == EntityBase::FREE_IN_3D.v) {
        ...
        ExprRef du = ua->Minus(ub);
        ExprRef dv = va->Minus(vb);

        ExprRef u, v;
        p->PointGetExprsInWorkplane(wrkpl, &u, &v);

        ExprRef m = ((du->Square())->Plus(dv->Square()))->Sqrt();

        ExprRef proj = (dv->Times(ua->Minus(u)))->Minus(
                     (du->Times(va->Minus(v))));
...
ExprRef ConstraintBase::PointPlaneDistance(ExprVector p, hEntity hpl) {
    ExprVector n;
    ExprRef d;
...
ExprRef ConstraintBase::Distance(hEntity wrkpl, hEntity hpa, hEntity hpb) {
    ...
    if(wrkpl.v == EntityBase::FREE_IN_3D.v) {
        ...
        ExprRef du = au->Minus(bu);
        ExprRef dv = av->Minus(bv);
...
//-----------------------------------------------------------------------------
ExprRef ConstraintBase::DirectionCosine(hEntity wrkpl,
                                      ExprVector ae, ExprVector be)
{
    if(wrkpl.v == EntityBase::FREE_IN_3D.v) {
        ExprRef mags = (ae.Magnitude())->Times(be.Magnitude());
        ...
        ExprRef ub = u.Dot(be);
        ExprRef vb = v.Dot(be);
        ExprRef maga = (ua->Square()->Plus(va->Square()))->Sqrt();
        ExprRef magb = (ub->Square()->Plus(vb->Square()))->Sqrt();
        ExprRef dot = (ua->Times(ub))->Plus(va->Times(vb));
...
void ConstraintBase::AddEq(IdList<Equation,hEquation> *l, ExprRef expr, int index)
{
    Equation eq;
    eq.e = expr;
...
void ConstraintBase::GenerateReal(IdList<Equation,hEquation> *l) {
    ...
    switch(type) {
        ...
        case EQ_LEN_PT_LINE_D: {
            ...
            ExprRef d1 = Distance(workplane, forLen->point[0], forLen->point[1]);
            ExprRef d2 = PointLineDistance(workplane, ptA, entityB);
            AddEq(l, (d1->Square())->Minus(d2->Square()), 0);
            break;
        }
        case EQ_PT_LN_DISTANCES: {
            ExprRef d1 = PointLineDistance(workplane, ptA, entityA);
            ExprRef d2 = PointLineDistance(workplane, ptB, entityB);
            AddEq(l, (d1->Square())->Minus(d2->Square()), 0);
            break;
        }
        ...
        case LENGTH_RATIO: {
            EntityBase *a = SK.GetEntity(entityA);
            EntityBase *b = SK.GetEntity(entityB);
            ExprRef la = Distance(workplane, a->point[0], a->point[1]);
            ExprRef lb = Distance(workplane, b->point[0], b->point[1]);
        ...
        case DIAMETER: {
            ...
            ExprRef r = circle->CircleGetRadiusExpr();
        ...
        case EQUAL_LINE_ARC_LEN: {
            ...
            ExprRef ll = (l1.Minus(l0)).Magnitude();
            ...
            ExprRef costheta = aof.Dot(u)->Div(r);
            ExprRef sintheta = aof.Dot(v)->Div(r);
        ...
        case PT_ON_CIRCLE: {
            ...
            ExprRef du = (center.Minus(pt)).Dot(u),
        ...
        case AT_MIDPOINT:
            if(workplane.v == EntityBase::FREE_IN_3D.v) {
                ...
                a->PointGetExprsInWorkplane(workplane, &au, &av);
                b->PointGetExprsInWorkplane(workplane, &bu, &bv);
                ExprRef mu = Expr::From(0.5)->Times(au->Plus(bu));
                ExprRef mv = Expr::From(0.5)->Times(av->Plus(bv));
        ...
        case SYMMETRIC_LINE: {
            ...
            ExprRef lau, lav, lbu, lbv;
            la->PointGetExprsInWorkplane(workplane, &lau, &lav);
            lb->PointGetExprsInWorkplane(workplane, &lbu, &lbv);

            ExprRef dpu = pbu->Minus(pau), dpv = pbv->Minus(pav);
            ExprRef dlu = lbu->Minus(lau), dlv = lbv->Minus(lav);
            ...
            ExprRef dista = (dlv->Times(lau->Minus(pau)))->Minus(
                          (dlu->Times(lav->Minus(pav))));
            ExprRef distb = (dlv->Times(lau->Minus(pbu)))->Minus(
        ...
        case SAME_ORIENTATION: {
            ...
            ExprRef d1 = au.Dot(bv);
            ExprRef d2 = au.Dot(bu);
        ...
        case ANGLE: {
            ...
            ExprRef c = DirectionCosine(workplane, ae, be);

            if(type == ANGLE) {
                // The direction cosine is equal to the cosine of the
                // specified angle
                ExprRef rads = exA->Times(Expr::From(PI/180));
        ...
        case EQUAL_ANGLE: {
            ...
            ExprRef cab = DirectionCosine(workplane, ae, be);
            ExprRef ccd = DirectionCosine(workplane, ce, de);
