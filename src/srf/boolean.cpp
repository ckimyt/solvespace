                                    ...
                                    SSurface *srfA, SSurface *srfB)
{
    ...
    auto p = pts.elem.begin();
    if(p == pts.elem.end()) oops();
    SCurvePt prev = *p;
    ret.pts.Add(*p);
    ++p;

    for(; p != pts.elem.end(); ++p) {
        List<SInter> il = {};
        ...
        if(il.Size() > 0) {
            ...
            Vector lineStart = prev.p;
            Vector lineDirection = (p->p).Minus(prev.p);
            std::sort(il.elem.begin(), il.elem.end(),
                [lineStart, lineDirection](SInter &a, SInter &b) {
...
void SSurface::TrimFromEdgeList(SEdgeList *el, bool asUv) {
    ...
    for(;;) {
        ...
        auto se = std::find_if(el->l.begin(), el->l.end(), [](SEdge *se) { return !se->tag; });
                                       ...
                                       int type)
{
    ...
    while(orig.l.Size() > 0) {
        ...
        SEdge *se = &(chain.l.elem[chain.l.Size()/2]);
    ...
    if(!final.AssemblePolygon(&poly, NULL, true)) {
        ...
        dbp("failed: I=%d, avoid=%d", I, choosing.l.Size());
...
void SShell::MakeFromAssemblyOf(SShell *a, SShell *b) {
    ...
    for(i = 0; i < 2; i++) {
        ...
        for(SSurface *s : ab->surface) {
            SSurface sn = SSurface::FromTransformationOf(s, t, q, 1.0, true);
...
void SShell::MakeFromBoolean(SShell *a, SShell *b, int type) {
    ...
    if(b->surface.Size() == 0 || a->surface.Size() == 0) {
        I = 1000000;
    } else {
        I = 0;
    }
...
SBspUv *SBspUv::From(SEdgeList *el, SSurface *srf) {
    ...
    std::sort(work.l.elem.begin(), work.l.elem.end(),
        [](SEdge &a, SEdge &b) {
